...
class auto_adb():
    ...
    def test_device(self):
        ...
        if output[0].decode('utf8') == 'List of devices attached\n\n':
            ...
            exit(1)
        print('设备已连接')
        # l = output[0].decode('utf8').split("\r\n")
        # for each in l:
        #     if "device" in each and "List" not in each:
        #         self.devices.append(each.replace("device", "").replace(" ", ""))
        #         print(each)
        # if len(self.devices) == 1:
        #     print("当前就一个设备 默认选择当前设备")
        # elif len(self.devices) > 1:
        #     print("\n\n有多个设备，请选择一个设备！")
        #     for inx, val in enumerate(self.devices):
        #         print(str(inx) + " : " + val)
        #     index = input("请输入设备编号索引：")
        #     self.device = self.devices[int(index)]
        #     self.adb_path = self.adb_path + " -s " + self.device
        #     print(self.adb_path)
